...
# варианты компенсатора для устранения артефактов экспозиции
...
# варианты методов для улучшения положения «камеры»
...
# 1.2.Математическое описание дескрипторов особых точек
# 2.1.1Вычисление детекторов ключевых точек на изображении
# Определение доступных методов и их конструкторов
...
# доступные методы для оценки трансформации (поворота)
...
# 1.3.Сопоставление особых точек
# 2.2. Поиск пар соответствующих особых точек.
# метод возвращающий объект для сопоставления наборов ключевых точек
def get_matcher(args):
    ...
    # https://github.com/opencv/opencv/blob/fc1a15626226609babd128e043cf7c4e32f567ca/modules/stitching/src/matchers.cpp
    # большая книжка стр 423
    if matcher_type == "affine":
        # https://docs.opencv.org/master/d3/dda/classcv_1_1detail_1_1AffineBestOf2NearestMatcher.html
        """
        https://docs.opencv.org/master/d3/dda/classcv_1_1detail_1_1AffineBestOf2NearestMatcher.html#details
        https://docs.opencv.org/master/da/d54/group__imgproc__transform.html
        """
        matcher = cv.detail_AffineBestOf2NearestMatcher(False, try_cuda, match_conf)
    elif range_width == -1:
        # https://docs.opencv.org/master/d4/d26/classcv_1_1detail_1_1BestOf2NearestMatcher.html
        """
        BestOf2NearestMatcher - это реализация критерия отношения второго ближайшего соседа Лоу,
        описанного в документе SIFT, где он рекомендует использовать ratio = 0.8.
        То же самое и для SURF. Иногда даже 0,85. Значение 0,3 является слишком строгим. 
        http://www.cs.technion.ac.il/users/wwwb/cgi-bin/tr-get.cgi/2016/MSC/MSC-2016-16.pdf
        https://www.uio.no/studier/emner/matnat/its/TEK5030/v19/lect/lecture_4_2_feature_matching.pdf
        """
        matcher = cv.detail.BestOf2NearestMatcher_create(try_cuda, match_conf)
    else:
        # https://docs.opencv.org/master/d8/d72/classcv_1_1detail_1_1BestOf2NearestRangeMatcher.html
        """
        В отличие от BestOf2NearestMatcher - тут есть параметр range_width, ограничивающий число попарных сравнений
        Это модификация метода от самой OpenCV
        """
...
def main(params=None):
    ...
    # последовательно проходим изображения
    for name in img_names:
        # Описание структуры программы
        # аналог void addImage(std::string file_path) – функция загружает изображение
        ...
        """
        # Это - извлечение особенностей, но тут и точки и дескрипторы выдаются вместе
        img_feat = cv.detail.computeImageFeatures2(finder, img)
        features.append(img_feat)
        img = cv.resize(src=full_img, dsize=None, fx=seam_scale, fy=seam_scale, interpolation=cv.INTER_LINEAR_EXACT)
        images.append(img)
    print("Изображения прочитаны, извлечены особенности, размеры рассчитаны")

    # 1.3.Сопоставление особых точек
    # 2.2. Поиск пар соответствующих особых точек.
    # вызов метода возвращающего объект для сопоставления наборов ключевых точек
    # передача ему ранее списков вычисленных ключевых точек для всех изображений
    matcher = get_matcher(args)
    p = matcher.apply2(features)
    matcher.collectGarbage()
    print("Особенности сопоставлены")
    ...
    """
    Фильтрация изображений, отсеивание неподходящих 
    http://www.compvision.ru/forum/index.php?/topic/1915-%D1%87%D1%82%D0%BE-%D1%82%D0%B0%D0%BA%D0%BE%D0%B5-leavebiggestcomponent/
    http://study.marearts.com/2013/12/finding-largest-subset-images-that-is.html
    https://stackoverflow.com/questions/39269798/opencv-3d-stitching-panorama
    с помощью leaveBiggestComponent - принимает все ключевые точки, все сопоставления, и пропускает только те изображения, которые точно являются частью панорамы
    """
    ...
    # ОЦЕНКА ГОМОГРАФИИ - определение метода
    estimator = ESTIMATOR_CHOICES[args.estimator]()
    # ОЦЕНКА ГОМОГРАФИИ - выполнение на особенностях
    """
    https://docs.opencv.org/master/df/d15/classcv_1_1detail_1_1Estimator.html
    вообще, "работа с камерами"
    альтернатива для множества наборов точек
    src_pts = np.float32([ kp1[m.queryIdx].pt for m in good ]).reshape(-1,1,2)
    dst_pts = np.float32([ kp2[m.trainIdx].pt for m in good ]).reshape(-1,1,2)
    M, mask = cv2.findHomography(src_pts, dst_pts, cv2.RANSAC, 5.0)
    Принимает все наборы особенностей всех изображений, все их сопоставления и оценивает вращение всех виртуальных "камер"
    """
    ...
    print("Маски перспективы (?) подсчитаны")

    images_warped_f = []
    for img in images_warped:
        imgf = img.astype(np.float32)
        images_warped_f.append(imgf)
    print("Перспективы применены к изображениям")

    compensator = get_compensator(args)
    compensator.feed(corners=corners, images=images_warped, masks=masks_warped)
    print("Компенсатор отработал")

    seam_finder = SEAM_FIND_CHOICES[args.seam]
    seam_finder.find(images_warped_f, corners, masks_warped)
    print("Искатель швов отработал")
    compose_scale = 1
    corners = []
    sizes = []
    blender = None
    ...
    for idx, name in enumerate(img_names):
        print("Изображение в работе: ", name)
        ...
        print("Изображение обработано: ", name)
    if not timelapse:
        result = None
        result_mask = None
